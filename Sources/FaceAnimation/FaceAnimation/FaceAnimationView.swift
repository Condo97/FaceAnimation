...
//        backgroundFaceLayer.path = originalBackgroundFacePath.cgPath
    ...
    func setupAnimations() {
        ...
        queue(faceAnimation: SmileIdleFaceAnimationStart())
        blink()
        queue(faceAnimation: SmileIdleFaceAnimation1())
        queue(faceAnimation: SmileIdleFaceAnimation2())
        queue(faceAnimation: WaitFaceAnimation(duration: 1.0))
        queue(faceAnimation: SmileIdleFaceAnimation1())
        blink()
        queue(faceAnimation: WaitFaceAnimation(duration: 1.0))
        queue(faceAnimation: SmileIdleFaceAnimation2())
        queue(faceAnimation: SmileIdleFaceAnimation3())
        queue(faceAnimation: SmileIdleFaceAnimationStart())
        blink()
        
        DispatchQueue.main.asyncAfter(deadline: .now() + 10) {
            self.queue(faceAnimation: ReadingLeftFaceAnimation())
            self.queue(faceAnimation: ReadingRightFaceAnimation())
            self.queue(faceAnimation: WaitFaceAnimation(duration: 1.0))
            self.queue(faceAnimation: ReadingLeftFaceAnimation())
            self.queue(faceAnimation: ReadingRightFaceAnimation())
        }
    }
